...
import { loadMapTranslationPopulation, type TranslationState } from "@/lib/server/map-translations";
...
type MapTargetStatusItem = {
  ...
  en_translation_status: TranslationState | null;
  he_translation_status: TranslationState | null;
};
...
async function loadAllMapTargets(supabase: SupabaseClient): Promise<MapTargetRow[]> {
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    ...
    if (error) {
      throw new Error(`Failed to load map_targets: ${error.message}`);
    }

    const batch = (data ?? []) as MapTargetRow[];
    rows.push(...batch);
...
async function loadAllMapStories(supabase: SupabaseClient): Promise<MapStoryRow[]> {
  const rows: MapStoryRow[] = [];
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    ...
    const batch = (data ?? []) as MapStoryRow[];
    rows.push(...batch);
...
async function loadAllSlidesForStories(
  supabase: SupabaseClient,
  storyIds: string[],
): Promise<MapStorySlideRow[]> {
  const rows: MapStorySlideRow[] = [];
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    ...
    const batch = (data ?? []) as MapStorySlideRow[];
    rows.push(...batch);
...
async function loadMapTargetsStatus(supabase: SupabaseClient): Promise<MapTargetStatusItem[]> {
  const [expectedTargets, typedTargets, typedStories] = await Promise.all([
    loadExpectedMapTargets(),
    loadAllMapTargets(supabase),
    loadAllMapStories(supabase),
  ]);
  ...
  if (storyIds.length > 0) {
    const slides = await loadAllSlidesForStories(supabase, storyIds);

    slidesByStoryId = slides.reduce((acc, slide) => {
      if (!slide.story_id) {
        return acc;
      }
  ...
  const items = Array.from(targetKeyMap.values()).map((target) => {
    ...
    return {
      ...
      en_translation_status: null,
      he_translation_status: null,
    };
  });
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const [items, translations] = await Promise.all([
      loadMapTargetsStatus(supabase),
      loadMapTranslationPopulation(supabase),
    ]);
    const statusByTarget = new Map(
      translations.rows.map((row) => [`${row.map_type}\u0000${row.target_id}`, row]),
    );
    const mergedItems = items.map((item) => {
      const translation = statusByTarget.get(`${item.map_type}\u0000${item.target_id}`);
      return {
        ...item,
        en_translation_status: translation?.en_status ?? null,
        he_translation_status: translation?.he_status ?? null,
      };
    });
    return res.status(200).json({ items: mergedItems, translation_summary: translations.summary });
  } catch (error) {
