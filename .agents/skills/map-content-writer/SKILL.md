...
- 🛡️ **Database-First Work Selection**: Отбор объектов для генерации осуществляется строго через СУБД VIEW `map_story_generation_queue`. Навык и скрипты НЕ имеют права самостоятельно скачивать `map_stories` и фильтровать объекты на стороне клиента. [`POLICY`]
